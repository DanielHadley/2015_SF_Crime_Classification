...
# & generate log-odds on training set with ~50% held out
...
# a few didn't work. let's replace with the one before
...
test_and_train$dates_numeric <- repeat.before(test_and_train$dates_numeric)

rm(date_and_time, repeat.before)
...
A_C_counts <- train[sample(nrow(train), .5 *nrow(train)), ] %>% 
...
write.csv(log_table_address_frequencies, "./log_table_address_frequencies_34.csv")
...
## Save the data ##
write.csv(test_and_train, "./test_and_train_34.csv", row.names = FALSE)
